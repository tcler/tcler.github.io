...
```
# /proc/self/mountinfo 个字段含义可以参考 proc(5) mount_namespaces(7)
...
第七列  optional fields
  shared:X
    his mount point is shared in peer group X.  Each peer group has a unique ID that is
    automatically generated by the kernel, and all mount points in the same  peer group
    will show the same ID.  (These IDs are assigned starting from the value 1, and may 
    be recycled when a peer group ceases to have any members.)
  ...
  propagate_from:X (since Linux 2.6.26)
    ...
    dominant peer group under the process's root directory.  If X is the immediate master
    of the mount, or if there is no dominant peer group  under  the  same  root, then
    only the master:X field is present and not the propagate_from:X field.  For further
    details, see below.
