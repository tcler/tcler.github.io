...
```
for i in {0..9}; do
    vm create 9 -n rdma-lab0-vm-49-$i --hostif eno12399v$i --nointeract --msize 6 --serial tcp,host=0.0.0.0:$((52445+i)),source.mode=bind --machine=q35 --boot=bootmenu.enable=on --boot=network,hd;
done

for i in {0..9}; do
    vm create 9 -n rdma-lab0-vm-46-$i --hostif eno12399v$i --nointeract --msize 6 --serial unix,path=/var/lib/libvirt/qemu/vm$((1+i)),source.mode=bind --machine=q35 --boot=bootmenu.enable=on --boot=network,hd;
done
```
