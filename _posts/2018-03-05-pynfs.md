...
rpm -q python-ply || {
  read -d . versionx versiony <<<$(lsb_release -sr)
  yum install https://dl.fedoraproject.org/pub/epel/epel-release-latest-${versionx}.noarch.rpm
...
mkdir -p /exportdir && echo '/exportdir *(rw,no_root_squash,insecure)' >/etc/exports && service nfs restart
