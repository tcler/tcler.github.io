...
server test
```
(cd /usr/src/pynfs/; python setup.py install)
cd /usr/src/pynfs/nfs4.0 # or cd pynfs/nfs4.1
...
mount -o vers=4.1 ${IP:-127.0.0.1}:/ ${nfsmp}
...
service nfs stop
(cd /usr/src/pynfs; python setup.py install)
cd /usr/src/pynfs/nfs4.1  # nfs4.1 开始支持 pnfs
